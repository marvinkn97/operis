  ...
  template: `
    <nav class="bg-white shadow-md px-6 py-3 flex justify-between items-center">
      ...
      <div class="hidden md:flex items-center gap-6">
        @if (isLoggedIn()) {
          <button (click)="onAiClick()" class="cursor-pointer">✨ AI Assistant</button>
          <a
            routerLink="/projects"
          ...
          <a routerLink="/tasks" class="text-gray-700 hover:text-blue-700 font-medium"> Tasks </a>
    ...
    @if (mobileMenuOpen() && isLoggedIn()) {
      <div class="md:hidden bg-white shadow-md px-6 py-4 flex flex-col gap-4">
        <button class="ai-button" (click)="onAiClick()">✨ AI Assistant</button>
        <a
          routerLink="/projects"
          (click)="toggleMenu()"
...
export class Navbar {
  ...
  onAiClick() {
    ...
    // 👇 THIS is the correct behavior (dialog, not page)
